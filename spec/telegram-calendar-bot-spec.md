...
- `remind me tmr at around 12pm to collect shopee parcel at j9` and `set a reminder tmr at 12pm to collect shopee parcel at j9` both create independent reminders. With a local reference time of 13 September 2026 at 10:20 PM, both are due 14 September at noon, without event selection or Calendar writes. A simple `today`, `tonight`, `tomorrow`, or `tmr` plus an AM/PM clock before `to` is resolved deterministically in the home timezone; dates inside the notification remain message text. Past due times are rejected with a request for a future time and no scheduler write.
