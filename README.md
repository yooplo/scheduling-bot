...
In the cron-job.org Console, open **Settings → API keys**, generate an API key, and store it in Render as `CRON_JOB_API_KEY`. Set `SERVICE_BASE_URL=https://YOUR-SERVICE.onrender.com`. The bot creates an expiring job for every independent reminder, enabling it only after the secured callback body is complete. The requested minute and following five minutes provide delivery opportunities. The `/scheduled/standalone-reminder` callback checks the absolute due time, sends Telegram, and deletes its job. New jobs have the `SchedulingBot reminder v2:` title prefix.

If the first scheduled minute is missed, a later attempt within the five-minute recovery window can deliver the reminder. Repeated sends are suppressed within the running process; restart or ambiguous network failures can still cause duplicates. A reminder's displayed time stays at its original due time. Existing missed jobs are not updated automatically: after deploying this change, remove obsolete jobs and recreate any reminders still needed. Job expiry stops execution but does not remove an undelivered job from the cron-job.org console. Delivery still depends on the scheduler and service being available within the recovery window.
